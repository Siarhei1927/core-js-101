...
function getSumBetweenNumbers(n1, n2) {
  const count = n2 - n1 + 1;
  const sum = ((n1 + n2) * count) / 2; // формула арифметической прогрессии
  return sum;
}
...
function doRectanglesOverlap(rect1, rect2) {
  const horizontal = rect1.left < rect2.left + rect2.width && rect1.left + rect1.width > rect2.left;
  const vertical = rect1.top < rect2.top + rect2.height && rect1.top + rect1.height > rect2.top;
...
function isInsideCircle(circle, point) {
  ...
  const distanceCheck = (x - center.x) ** 2 + (y - center.y) ** 2; // теорема Пифагора
  return distanceCheck < radius ** 2;
}
